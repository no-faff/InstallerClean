...
const MAP = {
  ...
  'Body.RegisteredMissingFromDisk': `이 설치 관리자 파일이 없습니다. 지금은 아무 문제도 일으키지 않으며, 이 파일이 속한 프로그램을 복구하거나 업데이트하거나 제거하려는 날이 오기 전까지는 문제가 되지 않습니다. 그때는 Windows가 이 파일을 찾는데 파일이 없으므로 그 단계가 실패할 수 있습니다.&#10;&#10;해결을 시도하려면 해당 프로그램의 설치 관리자를 제조사에서 내려받아 기존 설치본 위에 실행하세요(먼저 제거하지 마세요. 제거 자체가 이 파일을 필요로 하는 단계입니다). 구할 수 있다면 설치되어 있는 것과 같은 버전을 사용하세요. Windows가 다른 버전을 거부할 수 있습니다. 이렇게 하면 파일이 복구되고 설정은 그대로 유지될 것입니다. 다만 Microsoft가 보장하지는 않으며, Microsoft의 최후 수단은 프로그램을 다시 설치하는 것입니다.`,
  ...
  'Confirm.DeletePermanently.Plural': `파일들이 영구히 삭제됩니다. [안전하게 삭제할 수 있습니다]. 다만 백업을 원하시면 대신 이동 단추를 사용하세요.`,
