...
const MAP = {
  ...
  'Summary.SupersededHeldBack.Singular': `이 PC에서는 InstallerClean이 대체된 그 파일 하나가 더 이상 필요하지 않다고 확실히 알 수 없어서, 그 파일을 그대로 두었습니다.`,
  'Summary.SupersededHeldBack.Plural': `이 PC에서는 InstallerClean이 대체된 파일 {0}개가 더 이상 필요하지 않다고 확실히 알 수 없어서, 그 파일들을 그대로 두었습니다.`,
  'Cli.SupersededHeldBack.Singular': `이 PC에서는 InstallerClean이 대체된 그 파일 하나가 더 이상 필요하지 않다고 확실히 알 수 없어서, 그 파일을 그대로 두었습니다.`,
  'Cli.SupersededHeldBack.Plural': `이 PC에서는 InstallerClean이 대체된 파일 {0}개가 더 이상 필요하지 않다고 확실히 알 수 없어서, 그 파일들을 그대로 두었습니다.`,
  'Completion.HeldBack.Singular': `{0}개 파일을 보류했습니다. 검사는 필요 없다고 했지만, 최종 확인은 생각이 달랐습니다.`,
  'Completion.HeldBack.Plural': `{0}개 파일을 보류했습니다. 검사는 필요 없다고 했지만, 최종 확인은 생각이 달랐습니다.`,
  ...
  'Cli.NothingListed.Singular': `이 PC에서 InstallerClean은 캐시에 있는 어떤 파일이 여기 설치된 프로그램의 것인지 확신할 수 없어서, 그 파일 하나({2})를 목록에 올리지 않고 보류했습니다.`,
  'Cli.NothingListed.Plural': `이 PC에서 InstallerClean은 캐시에 있는 어떤 파일이 여기 설치된 프로그램의 것인지 확신할 수 없어서, {0} {1}({2})을 목록에 올리지 않고 보류했습니다.`,
  'Cli.WithheldReasons.Header': `확신할 수 없었던 이유:`,
  'Cli.WithheldReasons.RecordedPath': `  Windows Installer 자체 기록에 있는 파일 경로를 확인할 수 없었습니다.`,
  'Cli.WithheldReasons.FileIdentity': `  Windows Installer 기록에 나온 파일의 신원을 읽을 수 없었습니다.`,
  'Cli.WithheldReasons.SecondInstance': `  이 PC에 같은 프로그램이 두 번 이상 설치되어 있을 수 있습니다.`,
