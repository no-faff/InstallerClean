...
const MAP = {
  ...
  'Action.MoveInstead': `_Перемістити натомість`,
  'Action.MoveDestinationPlaceholder': `Шлях до папки, якщо ви переміщуєте замість видалення`,
  ...
  'Automation.RecycleUnavailable': `Виберіть, що робити з непотрібними файлами: перемістити їх у безпечне місце, видалити назавжди або скасувати.`,
  'Automation.RecycleUnavailableMove': `Перемістити непотрібні файли до обраної вами папки`,
  'Automation.RecycleUnavailableDeletePermanently': `Видалити непотрібні файли назавжди, бо Кошик недоступний для цього диска`,
  ...
  'Completion.MoveRestoreHint': `Скопіюйте їх назад у {InstallerFolder}, якщо раптом щось зламається ([вкрай малоймовірно]).`,
  'Completion.DeleteRestoreHint': `А поки що ви можете відновити їх, якщо раптом щось зламається ([вкрай малоймовірно]).`,
  ...
  'Completion.DeleteSummary.Singular': `{0} {1} переміщено до Кошика`,
  'Completion.DeleteSummary.Plural': `{0} {1} переміщено до Кошика`,
  ...
  'RecycleUnavailable.Heading': `Кошик недоступний для цього диска`,
  'RecycleUnavailable.Body.Singular': `Тож цей {1} ({2}) не видалено. Ви можете перемістити його в безпечне місце або видалити назавжди.`,
  'RecycleUnavailable.Body.Plural': `Тож ці {0} {1} ({2}) не видалено. Ви можете перемістити їх у безпечне місце або видалити назавжди.`,
  'RecycleUnavailable.Reassurance.Singular': `Видаляти його безпечно. InstallerClean прибирає лише ті файли, які Windows позначає як завершені, ніколи той, що ще потрібен програмі, а Кошик — це лише додатковий запобіжник. У малоймовірному разі, якщо видалення колись лишило програму нездатною відновитися, оновитися чи видалитися, перевстановлення її від виробника зазвичай повертає файл, хоча Microsoft цього не гарантує.`,
  'RecycleUnavailable.Reassurance.Plural': `Видаляти їх безпечно. InstallerClean прибирає лише ті файли, які Windows позначає як завершені, ніколи той, що ще потрібен програмі, а Кошик — це лише додатковий запобіжник. У малоймовірному разі, якщо видалення колись лишило програму нездатною відновитися, оновитися чи видалитися, перевстановлення її від виробника зазвичай повертає файл, хоча Microsoft цього не гарантує.`,
  ...
  'Confirm.DeleteToRecycleBin': `Файли буде переміщено до Кошика. Якщо ви хочете зберегти резервні копії, скористайтеся кнопкою «Перемістити».`,
  ...
  // 0 = shell error code
  'Error.ShellRecycleFailed': `Не вдалося перемістити цей файл до Кошика (помилка {0}), і за цим кодом InstallerClean не може сказати чому. Файл залишено на місці. Спробуйте натомість кнопку «Перемістити», яка не використовує Кошик.`,
  'Error.RecycleAccessDenied': `Windows відмовив у доступі навіть із правами адміністратора (помилка {0}), і InstallerClean не може визначити, у чому річ — у файлі чи в Кошику. Файл залишено на місці. Кнопка «Перемістити» допоможе, якщо річ у Кошику, але не допоможе, якщо річ у файлі.`,
  'Error.RecycleInUse': `Цей файл відкритий або заблокований іншою програмою (помилка {0}), тож зараз його ніщо не може видалити. Його залишено на місці; спробуйте ще раз пізніше.`,
  'Error.DeletedNotRecycled': `Windows видалив цей файл безповоротно, а не перемістив його до Кошика. InstallerClean попросив Кошик, а Windows вчинив інакше. Файла більше немає.`,
  ...
  'Cli.RecycleUnavailable': `Помилка: Кошик недоступний для цього тому, тож нічого не видалено. Скористайтеся /m, щоб перемістити файли натомість, або знову ввімкніть Кошик і запустіть ще раз.`,
  ...
  'Cli.Help.ScanOnly': `  installerclean-cli /s         Лише сканувати, перелічити непотрібні файли`,
  'Cli.Help.Delete': `  installerclean-cli /d         Видалити непотрібні файли (Кошик)`,
  'Cli.Help.MoveDefault': `  installerclean-cli /m         Перемістити в збережене типове розташування`,
  ...
  'Cli.Help.NoteLine3': `іншого консольного exe. Графічний інтерфейс міститься поруч, у InstallerClean.exe.`,
  'Cli.Help.ExitCodesHeader': `Коди виходу:`,
  'Cli.Help.ExitCodeOk': `  0   успіх: оброблено кожен позначений файл`,
  'Cli.Help.ExitCodeError': `  1   невдача: нічого не оброблено (хибні аргументи, збій сканування, усі файли зазнали збою)`,
  ...
  'Completion.CleanedUp': `Очищено {0}`,
  'Completion.DeleteSpaceHint': `Очистіть Кошик, щоб справді звільнити місце.`,
  ...
  'Completion.DeleteCancelledSummary': `Переміщено {0} з {1} {2} до Кошика, перш ніж ви скасували.`,
  ...
  'Cli.Help.MoveScheduledNote': `Збережене типове розташування діє лише для поточного користувача; для запусків за розкладом чи від імені SYSTEM потрібно вказати /m ШЛЯХ.`,
