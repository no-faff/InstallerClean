...
const MAP = {
  ...
  'Body.MainExplanation.Action': `Переместите их в выбранную вами папку резервных копий, а затем удалите эту папку, когда убедитесь, что ваши программы по-прежнему обновляются и удаляются как обычно. Возвращение их в {InstallerFolder} восстанавливает всё. Или удалите их безвозвратно прямо сейчас.`,
  ...
  'Body.RegisteredMissingFromDisk': `Этот установочный файл отсутствует. Сейчас это не доставляет хлопот и не будет доставлять до того дня, когда вы попробуете обновить или удалить программу, которой он принадлежит. Тогда этот шаг может не выполниться, потому что Windows ищет этот файл и не находит его.\n\nЧтобы вернуть его, вам нужен установщик той версии, которая у вас уже есть. Возьмите его у изготовителя программы и запустите поверх имеющейся копии. Более новая версия не подойдёт: ей пришлось бы сначала удалить ту, что у вас есть, а именно этому шагу и нужен этот файл. Удалить сначала тоже не получится, по той же причине. Это должно восстановить файл и оставить ваши настройки нетронутыми, но Microsoft этого не гарантирует.`,
  ...
  'Status.MoveFailed': `{0}. Подробности в {1}.`,
  'Status.MoveFailed.NoLog': `{0}. Не удалось записать журнал сбоев.`,
  'Status.DeleteFailed': `{0}. Подробности в {1}.`,
  'Status.DeleteFailed.NoLog': `{0}. Не удалось записать журнал сбоев.`,
  ...
  'Error.StoppedTitle': `InstallerClean остановился`,
  ...
  'Error.MsiNonSuccess': `Windows Installer не смог предоставить InstallerClean читаемый список установленных программ: он прочитал {1} {2}, а затем одна запись вернулась нечитаемой (код ошибки {0}). Вместо того чтобы работать с прочитанным лишь частично списком, InstallerClean остановился. Ничего не было удалено.`,
  ...
  'UpdateCheck.Failed.Unknown': `Проверка не удалась по неизвестной причине. Подробности в {0}, если нужно сообщить об этом.`,

  // 0 = the URL the user was trying to reach
  'BrowserLaunch.ClipboardOk': `Ссылка в буфере обмена, так что вы можете вставить её сами:&#10;&#10;{0}`,
  'BrowserLaunch.ClipboardFailed': `InstallerClean также не смог скопировать ссылку в буфер обмена, вот она:&#10;&#10;{0}`,
  ...
  'Startup.AlreadyRunningTitle': `InstallerClean`,
  'Startup.AlreadyRunningBody': `Уже запущен.`,
  'Startup.UnhandledTitle': `InstallerClean`,
  ...
  'Completion.MoveCancelledSummary': `Перемещено {0}/{1} {2} в {3} до отмены.`,
  ...
  'Tooltip.MoveSameDrive': `Перемещает ненужные файлы в папку резервных копий. Она на том же диске, поэтому место освободится только после того, как вы удалите эту папку.`,
  'Confirm.DeletePermanently.Singular': `Этот файл будет удалён безвозвратно. Это безопасно, но если хотите резервную копию, воспользуйтесь «Переместить».`,
  'Confirm.DeletePermanently.Plural': `Эти файлы будут удалены безвозвратно. Это безопасно, но если хотите резервную копию, воспользуйтесь «Переместить».`,
  ...
  'Error.ScanInstallerFolderElsewhere': `InstallerClean не может просканировать {InstallerFolder} на этом ПК. Windows сообщает, что эта папка находится в {0}, а InstallerClean сканирует папку Installer, только если Windows указывает для неё её собственный путь. Ничего не предложено и ничего не убрано.`,
  ...
  'Body.PendingReboot.RegistryCheckUnreadable': `InstallerClean не смог прочитать один из параметров Windows, которые проверяет перед тем, как трогать {InstallerFolder}, поэтому не может определить, выполняется ли операция установщика или ожидает перезагрузки. Перезагрузите Windows и повторите сканирование. Если параметр по-прежнему не читается, это не та машина, которую InstallerClean может очистить.`,
  'Completion.MoveCancelledRestoreHint': `Это легко отменить. Переместите их обратно в {InstallerFolder}, и всё вернётся как было.`,
  'Error.InstallerLockAccessRefused': `Windows отказал InstallerClean в праве проверить, занят ли Windows Installer, поэтому он не смог исключить, что файл понадобится в процессе, и ничего не было удалено.`,
  'Error.MoveInstallerLockAccessRefused': `Windows отказал InstallerClean в праве проверить, занят ли Windows Installer, поэтому он не смог исключить, что файл понадобится в процессе, и ничего не было перемещено.`,
  'Error.MoveStoppedTitle': `Перемещение остановлено`,
  'Field.NoNamedProduct': `(нет программы)`,
...
const CLI = {
  ...
  'Cli.MissingFromDisk.Singular': `У Windows есть запись о {0} файле, которого нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но обновление или удаление этой программы может не выполниться. Чтобы вернуть файл, вам нужен установщик той версии, которая у вас уже есть. Возьмите его у изготовителя программы и запустите поверх имеющейся копии. Более новая версия не подойдёт: ей пришлось бы сначала удалить ту, что у вас есть, а именно этому шагу и нужен этот файл. Удалить сначала тоже не получится, по той же причине. Это должно восстановить файл и оставить ваши настройки нетронутыми, но Microsoft этого не гарантирует.`,
  'Cli.MissingFromDisk.Plural': `У Windows есть записи о {0} файлах, которых нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но обновление или удаление этих программ может не выполниться. Чтобы вернуть файл, вам нужен установщик той версии этой программы, которая у вас уже есть. Возьмите его у изготовителя программы и запустите поверх имеющейся копии. Более новая версия не подойдёт: ей пришлось бы сначала удалить ту, что у вас есть, а именно этому шагу и нужен этот файл. Удалить сначала тоже не получится, по той же причине. Это должно восстановить файл и оставить ваши настройки нетронутыми, но Microsoft этого не гарантирует.`,
