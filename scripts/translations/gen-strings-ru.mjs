...
const MAP = {
  ...
  'Action.MoveInstead': `_Переместить вместо этого`,
  ...
  'Automation.RecycleUnavailable': `Выберите, что сделать с ненужными файлами: переместить в надёжное место, удалить безвозвратно или отменить.`,
  'Automation.RecycleUnavailableMove': `Переместить ненужные файлы в выбранную вами папку`,
  'Automation.RecycleUnavailableDeletePermanently': `Удалить ненужные файлы безвозвратно, потому что Корзина недоступна для этого диска`,
  ...
  'Completion.MoveRestoreHint': `Скопируйте их обратно в {InstallerFolder}, если вдруг что-то сломается ([крайне маловероятно]).`,
  'Completion.DeleteRestoreHint': `А пока их можно восстановить, если вдруг что-то сломается ([крайне маловероятно]).`,
  ...
  'Completion.DeleteSummary.Singular': `Перемещено {0} {1} в Корзину`,
  'Completion.DeleteSummary.Plural': `Перемещено {0} {1} в Корзину`,
  ...
  'Confirm.DeleteToRecycleBin': `Файлы будут перемещены в Корзину. Если хотите сделать резервные копии, воспользуйтесь кнопкой «Переместить».`,
  ...
  // 0 = shell error code
  'Error.ShellRecycleFailed': `Не удалось переместить этот файл в Корзину (ошибка {0}), и по этому коду InstallerClean не может сказать почему. Файл оставлен на месте. Попробуйте вместо этого кнопку «Переместить», которая не использует Корзину.`,
  'Error.RecycleAccessDenied': `Windows отказал в доступе даже с правами администратора (ошибка {0}), и InstallerClean не может определить, в чём дело — в файле или в Корзине. Файл оставлен на месте. Кнопка «Переместить» поможет, если дело в Корзине, но не поможет, если дело в файле.`,
  'Error.RecycleInUse': `Этот файл открыт или заблокирован другой программой (ошибка {0}), поэтому сейчас его ничто не может удалить. Он оставлен на месте; повторите попытку позже.`,
  'Error.DeletedNotRecycled': `Windows удалил этот файл сразу и безвозвратно, вместо того чтобы переместить его в Корзину. InstallerClean запросил Корзину, а Windows поступил иначе. Файла больше нет.`,
  ...
  'Completion.DeleteSpaceHint': `Очистите Корзину, чтобы действительно освободить место.`,
...
const CLI = {
  ...
  'Cli.Help.Help': `  installerclean-cli --help       Показать эту справку (также принимает /?, -h)`,
  'Cli.Help.Version': `  installerclean-cli --version    Показать версию (также принимает -v)`,
  'Cli.Help.ScanOnly': `  installerclean-cli /s           Только сканирование - список ненужных файлов`,
  'Cli.Help.Delete': `  installerclean-cli /d           Удалить ненужные файлы (Корзина)`,
  'Cli.Help.MoveDefault': `  installerclean-cli /m           Переместить в сохранённую папку по умолчанию`,
  ...
  'Cli.Help.NoteLine3': `по конвейеру, как у любого консольного exe. GUI рядом, в InstallerClean.exe.`,
  'Cli.Help.ExitCodesHeader': `Коды выхода:`,
  'Cli.Help.ExitCodeOk': `  0   успех: обработаны все отмеченные файлы`,
  'Cli.Help.ExitCodeError': `  1   ошибка: ничего не обработано (неверные аргументы, сбой сканирования, все файлы с ошибкой)`,
  ...
  'Cli.Help.MoveScheduledNote': `Сохранённое значение по умолчанию задаётся отдельно для каждого пользователя; для запусков по расписанию или от имени SYSTEM нужно указывать /m ПУТЬ.`,
};
