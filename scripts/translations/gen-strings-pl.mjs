...
const OVERRIDES = {
  ...
  'Completion.HeldBack.Few': `Zatrzymano {0} pliki. Skanowanie uznało je za niepotrzebne. Końcowa kontrola się z tym nie zgodziła.`,
  'Summary.SupersededHeldBack.Few': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że {0} zastąpione pliki nie są już potrzebne, więc je zatrzymał.`,
  'Cli.SupersededHeldBack.Few': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że {0} zastąpione pliki nie są już potrzebne, więc je zatrzymał.`,
};
...
const MAP = {
  ...
  'Summary.SupersededHeldBack.Singular': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że jedyny zastąpiony plik nie jest już potrzebny, więc go zatrzymał.`,
  'Summary.SupersededHeldBack.Plural': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że {0} zastąpionych plików nie jest już potrzebnych, więc je zatrzymał.`,
  'Cli.SupersededHeldBack.Singular': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że jedyny zastąpiony plik nie jest już potrzebny, więc go zatrzymał.`,
  'Cli.SupersededHeldBack.Plural': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, że {0} zastąpionych plików nie jest już potrzebnych, więc je zatrzymał.`,
  'Completion.HeldBack.Singular': `Zatrzymano {0} plik. Skanowanie uznało go za niepotrzebny. Końcowa kontrola się z tym nie zgodziła.`,
  'Completion.HeldBack.Plural': `Zatrzymano {0} plików. Skanowanie uznało je za niepotrzebne. Końcowa kontrola się z tym nie zgodziła.`,
  ...
  'Cli.NothingListed.Singular': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, które pliki w pamięci podręcznej należą do zainstalowanych tu programów, więc zatrzymał ten jeden plik ({2}), zamiast go wypisać.`,
  'Cli.NothingListed.Plural': `Na tym komputerze InstallerClean nie zdołał ustalić z pewnością, które pliki w pamięci podręcznej należą do zainstalowanych tu programów, więc zatrzymał {0} {1} ({2}), zamiast je wypisać.`,
  'Cli.WithheldReasons.Header': `Dlaczego nie było pewności:`,
  'Cli.WithheldReasons.RecordedPath': `  Nie udało się rozwiązać ścieżki pliku z własnych rejestrów Windows Installer.`,
  'Cli.WithheldReasons.FileIdentity': `  Nie udało się odczytać tożsamości pliku wymienionego w rejestrach Windows Installer.`,
  'Cli.WithheldReasons.SecondInstance': `  Program może być zainstalowany na tym komputerze więcej niż raz.`,
