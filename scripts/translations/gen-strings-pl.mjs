...
const MAP = {
  ...
  'Body.PendingReboot.MsiExecuteMutexAccessRefused': `Windows odmówił InstallerClean uprawnień do sprawdzenia, czy Instalator Windows jest zajęty, więc Przenieś i Usuń są wstrzymane, a InstallerClean nie ruszy {InstallerFolder}. Uruchom Windows ponownie i skanuj ponownie. Jeśli Windows nadal odmawia, to nie jest maszyna, którą InstallerClean może wyczyścić.`,
  ...
  'Completion.FailedCount.Singular': `Nie udało się przenieść {0} pliku.`,
  'Completion.FailedCount.Plural': `Nie udało się przenieść {0} plików.`,
  'Completion.FailedCountDelete.Singular': `Nie udało się usunąć {0} pliku.`,
  'Completion.FailedCountDelete.Plural': `Nie udało się usunąć {0} plików.`,
  ...
  'Error.StoppedTitle': `InstallerClean się zatrzymał`,
  ...
  'Error.MsiNonSuccess': `Windows Installer nie zdołał przekazać InstallerClean czytelnej listy zainstalowanych programów: odczytał {1} {2}, a potem jeden wpis wrócił nieczytelny (kod błędu {0}). Zamiast pracować na liście odczytanej tylko częściowo, InstallerClean się zatrzymał. Nic nie zostało usunięte.`,
  ...
  'Completion.MoveCancelledSummary': `Przed anulowaniem przeniesiono {0}/{1} {2} do {3}.`,
  ...
  'Error.ScanInstallerFolderElsewhere': `InstallerClean nie może przeskanować {InstallerFolder} na tym komputerze. Windows podaje ten folder jako {0}, a InstallerClean skanuje tylko folder Installer, który Windows podaje pod jego własną ścieżką. Niczego nie zaproponowano i niczego nie usunięto.`,
