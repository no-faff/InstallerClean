...
const MAP = {
  ...
  'Tooltip.SigningCertificate': `来自内嵌 Authenticode 证书的主题名称。未验证证书链。`,
  ...
  'Cli.RecycleUnavailable': `错误：此驱动器的回收站不可用，因此未删除任何内容。请改用 /m 移动这些文件，或重新启用回收站后再次运行。`,
  ...
  'Cli.MutexBlocked': `另一个 InstallerClean 进程正持有单实例锁（GUI 或另一次 CLI 运行）。退出码 75（暂时性）；稍后可安全重试。`,
