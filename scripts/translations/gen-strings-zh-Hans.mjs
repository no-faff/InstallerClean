...
const MAP = {
  ...
  'Tooltip.Move': `把不需要的文件移动到备份文件夹。等您确信没有任何东西需要它们时，再删除那个文件夹。`,
  'Tooltip.MoveNeedsDestination': `把不需要的文件移动到一个备份文件夹。您接下来会选择它。等您确信没有任何东西需要它们时，再删除那个文件夹。`,
  ...
  'Tooltip.MoveSameDrive': `把不需要的文件移动到备份文件夹。它在同一个驱动器上，所以要等您删除那个文件夹、或把它移到另一个驱动器，才会收回空间。等您确信没有任何东西需要它们时就可以这么做。`,
  ...
  'Cli.DestinationChangedMidBatch': `InstallerClean 已无法确认备份文件夹，因此停了下来，而不是写到错误的位置。请检查 {0}，然后重新运行该命令。`,
