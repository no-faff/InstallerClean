...
const MAP = {
  ...
  'Tooltip.Move': `把不需要的文件移动到目标文件夹。等您确信没有任何东西需要它们时，再删除那个文件夹。`,
  'Tooltip.MoveNeedsDestination': `把不需要的文件移动到一个目标文件夹。您接下来会选择它。等您确信没有任何东西需要它们时，再删除那个文件夹。`,
  'Tooltip.Delete': `永久删除不需要的文件。它们可以安全移除，而且空间会立刻回来。`,
  ...
  'Body.MainExplanation.Action': `把它们移动到您自己选择的目标文件夹，等确认程序照常更新、修复和卸载之后，再删除那个文件夹。把它们放回 {InstallerFolder} 就能恢复一切。或者现在就永久删除。`,
  ...
  'Body.RegisteredMissingFromDisk': `这个安装程序文件缺失了。目前不会造成任何麻烦，直到有一天您要修复、更新或卸载它所属的程序为止。那一步可能会失败，因为 Windows 会去找这个文件，而它不在。&#10;&#10;想要修复，请从该程序的厂商处下载它的安装程序，并在现有安装之上运行（不要先卸载：卸载本身就是需要这个文件的步骤）。如果能拿到，请使用您已安装的那个版本，因为 Windows 可能会拒绝其他版本。这应该能恢复该文件并保留您的设置，但 Microsoft 并不保证，而它自己的最后手段是重新安装该程序。`,
  ...
  'Error.DestinationNotFullyQualified': `目标文件夹必须是指向某个文件夹的完整路径，以驱动器盘符或网络共享开头（例如 D:\\Backup，或 \\\\server\\backup）。InstallerClean 无法使用这个：{0}`,
  ...
  'Error.DestinationChangedMidBatch': `InstallerClean 已无法确认目标文件夹，因此停了下来，而不是写到错误的位置。请检查 {0}，然后重新扫描并再试一次。`,
  ...
  'Cli.Help.MoveDefault': `  installerclean-cli /m         移动到已保存的目标文件夹`,
  ...
  'Tooltip.MoveSameDrive': `把不需要的文件移动到目标文件夹。它在同一个驱动器上，所以要等您删除那个文件夹、或把它移到另一个驱动器，才会收回空间。等您确信没有任何东西需要它们时就可以这么做。`,
  'Completion.MoveRestoreHint.Singular': `那个文件夹里的文件[可以安全移除]，所以您想什么时候删除文件夹都可以。在那之前，万一有程序确实需要它，您可以把它放回 {InstallerFolder}（可能性极小）。`,
  'Completion.MoveRestoreHint.Plural': `那个文件夹里的文件[可以安全移除]，所以您想什么时候删除它都可以。在那之前，万一有程序确实需要其中某一个，您可以把它们放回 {InstallerFolder}（可能性极小）。`,
  'Completion.MoveRestoreHintSameDrive.Singular': `那个文件夹里的文件[可以安全移除]，所以当您真的想收回空间时，删除该文件夹或把它移到另一个驱动器都可以。在那之前，万一有程序确实需要它，您可以把它放回 {InstallerFolder}（可能性极小）。`,
  'Completion.MoveRestoreHintSameDrive.Plural': `那个文件夹里的文件[可以安全移除]，所以当您真的想收回空间时，删除该文件夹或把它移到另一个驱动器都可以。在那之前，万一有程序确实需要其中某一个，您可以把它们放回 {InstallerFolder}（可能性极小）。`,
  'Confirm.DeletePermanently.Singular': `此文件将被永久删除。它[可以安全删除]，但如果您想要备份，请改用“移动”按钮。`,
  'Confirm.DeletePermanently.Plural': `这些文件将被永久删除。它们[可以安全删除]，但如果您想要备份，请改用“移动”按钮。`,
  ...
  'Cli.MissingFromDisk.Singular': `Windows 有 {0} 个不在 {InstallerFolder} 中的文件的记录：{1}。日常使用不会有麻烦，但修复、更新或卸载可能因此失败。重新运行该程序的安装程序，最好是同一版本，通常能恢复这个文件。`,
  'Cli.MissingFromDisk.Plural': `Windows 有 {0} 个不在 {InstallerFolder} 中的文件的记录：{1}。日常使用不会有麻烦，但修复、更新或卸载可能因此失败。重新运行各个程序的安装程序，最好是同一版本，通常能恢复这些文件。`,
  ...
  'Cli.NothingOffered.Singular': `InstallerClean 无法确定缓存中的哪些文件属于这里安装的程序，因此保留了本可提供的那 1 个文件（{2}）。`,
  'Cli.NothingOffered.Plural': `InstallerClean 无法确定缓存中的哪些文件属于这里安装的程序，因此保留了本可提供的全部 {0} 个{1}（{2}）。`,
  'Cli.DestinationChangedMidBatch': `InstallerClean 已无法确认目标文件夹，因此停了下来，而不是写到错误的位置。请检查 {0}，然后重新运行该命令。`,
  ...
  'Summary.RecordsNotMatched': `InstallerClean 未能把 Windows 记录中的每一项都对应上，因此没有全部读取。上面那些不需要的文件不受影响，但关于 {InstallerFolder} 中缺失文件的说明可能不完整。请重新扫描再试一次。`,
  'Cli.RecordsNotMatched': `InstallerClean 未能把 Windows 记录中的每一项都对应上，因此没有全部读取。已找到的内容不受影响，但关于 {InstallerFolder} 中缺失文件的说明可能不完整。再运行一次也许能找到更多。`,
