...
const MAP = {
  ...
  'Automation.BuyMeACuppa.About': `寄付`,
  'Automation.Donate': `寄付`,
  ...
  'Tooltip.BuyMeACuppa.About': `どんな少額のご寄付でもありがたく思います。`,
  ...
  'Status.MoveFailed': `{0}。詳細は{1}をご覧ください。`,
  'Status.MoveFailed.NoLog': `{0}。クラッシュログを書き込めませんでした。`,
  'Status.DeleteFailed': `{0}。詳細は{1}をご覧ください。`,
  'Status.DeleteFailed.NoLog': `{0}。クラッシュログを書き込めませんでした。`,
  ...
  'Completion.FailedCount.Singular': `{0} 個のファイルを移動できませんでした。`,
  'Completion.FailedCount.Plural': `{0} 個のファイルを移動できませんでした。`,
  'Completion.FailedCountDelete.Singular': `{0} 個のファイルを削除できませんでした。`,
  'Completion.FailedCountDelete.Plural': `{0} 個のファイルを削除できませんでした。`,
  ...
  'Summary.NothingListed.Singular': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、1 個のファイルを提示せずに保留しました。`,
  'Summary.NothingListed.Plural': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、{0} 個の{1}を提示せずに保留しました。`,
  ...
  'Error.StoppedTitle': `InstallerClean は中止しました`,
  ...
  'BrowserLaunch.ClipboardOk': `リンクはクリップボードにあります。ご自身で貼り付けてください：&#10;&#10;{0}`,
  'BrowserLaunch.ClipboardFailed': `InstallerClean はリンクをクリップボードにコピーすることもできませんでした。こちらです：&#10;&#10;{0}`,
  ...
  'Completion.DonateAsk': `どんな少額のご寄付でもありがたく思います。`,
  ...
  'Cli.NothingOffered.Singular': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、1 個のファイル({2})を提示せずに保留しました。`,
  ...
  'Error.ScanInstallerFolderElsewhere': `InstallerClean はこの PC の {InstallerFolder} をスキャンできません。Windows はこのフォルダーの場所を {0} と報告しており、InstallerClean がスキャンするのは、Windows がそのフォルダー自身のパスで報告する Installer フォルダーだけです。何も提示されず、何も取り除かれていません。`,
  'Completion.NothingOffered': `この PC では何も提示されませんでした`,
  'Completion.NothingOfferedBody.Singular': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、1 個のファイル({2})を提示せずに保留しました。`,
  'Completion.NothingOfferedBody.Plural': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、{0} 個の{1}({2})をすべて提示せずに保留しました。`,
  'Summary.SupersededHeldBack.Singular': `InstallerClean は唯一の置換済みファイルがもう不要だと確実には判断できなかったため、そのファイルを保留しました。`,
  'Summary.SupersededHeldBack.Plural': `InstallerClean は {0} 個の置換済みファイルがもう不要だと確実には判断できなかったため、それらのファイルを保留しました。`,
  'Cli.SupersededHeldBack.Singular': `InstallerClean は唯一の置換済みファイルがもう不要だと確実には判断できなかったため、そのファイルを保留しました。`,
  'Cli.SupersededHeldBack.Plural': `InstallerClean は {0} 個の置換済みファイルがもう不要だと確実には判断できなかったため、それらのファイルを保留しました。`,
  'Completion.HeldBack.Singular': `{0} 件のファイルを保留しました。スキャンでは不要と判断されましたが、最終チェックではそれを確認できませんでした。`,
  'Completion.HeldBack.Plural': `{0} 件のファイルを保留しました。スキャンでは不要と判断されましたが、最終チェックではそれを確認できませんでした。`,
  ...
  'Cli.NothingListed.Singular': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、1 個のファイル({2})を提示せずに保留しました。`,
  'Cli.NothingListed.Plural': `InstallerClean は、キャッシュ内のどのファイルがここにインストールされたプログラムのものかを確実には判断できなかったため、{0} 個の{1}({2})を提示せずに保留しました。`,
  'Cli.WithheldReasons.Header': `確信を持てなかった理由:`,
  'Cli.WithheldReasons.RecordedPath': `  Windows Installer 自身の記録にあるファイルパスを解決できなかったため、それに何も照合できませんでした。`,
  'Cli.WithheldReasons.FileIdentity': `  Windows に記録のあるファイルを識別できなかったため、フォルダー内のものと照合できませんでした。`,
  'Cli.WithheldReasons.SecondInstance': `  この PC には同じプログラムが複数回インストールされている可能性があり、記録ではファイルがどのコピーのものか分かりません。`,
  ...
  'Cli.NothingOfferedPerFile.Plural': `InstallerClean は、見つけたキャッシュ内のファイルのいずれについても不要であることを確認できなかったため、{0} 個の{1}({2})をすべて提示せずに保留しました。`,
  'Completion.NothingOfferedPerFileBody.Singular': `InstallerClean は、見つけたキャッシュ内のファイルが不要であることを確認できなかったため、その 1 個のファイル({2})を提示せずに保留しました。`,
  'Completion.NothingOfferedPerFileBody.Plural': `InstallerClean は、見つけたキャッシュ内のファイルのいずれについても不要であることを確認できなかったため、{0} 個の{1}({2})をすべて提示せずに保留しました。`,
  ...
  'Cli.MoveInstallerLockAccessRefused': `エラー: Windows は InstallerClean に Windows Installer が処理中かどうかを確認する権限を与えなかったため、途中でファイルが必要になる可能性を排除できませんでした。何も移動されていません。`,
  'Cli.PendingRebootBlocked.RegistryCheckUnreadable': `エラー: InstallerClean は {InstallerFolder} に触れる前に確認するレジストリ値の一つを読み取れなかったため、実行中または次回の再起動に予約された Windows Installer の操作を除外できません。/m と /d はブロックされています。Windows を再起動してから再試行してください。それでも読み取りに失敗する場合、この PC は InstallerClean がクリーンアップできる PC ではありません。`,
  'Completion.MoveCancelledRestoreHint': `元に戻すのは簡単です。{InstallerFolder} に戻せば、すべて元どおりになります。`,
  'Error.InstallerLockAccessRefused': `Windows は InstallerClean に Windows Installer が処理中かどうかを確認する権限を与えなかったため、途中でファイルが必要になる可能性を排除できず、何も削除されませんでした。`,
  'Error.MoveInstallerLockAccessRefused': `Windows は InstallerClean に Windows Installer が処理中かどうかを確認する権限を与えなかったため、途中でファイルが必要になる可能性を排除できず、何も移動されませんでした。`,
  'Error.MoveStoppedTitle': `移動を停止しました`,
  'Field.NoNamedProduct': `(プログラムなし)`,
...
const STRIPPED = new Set([
  ...
  'Cli.EventLogMissingFromDisk.Singular',
  'Cli.EventLogMissingFromDisk.Plural',
