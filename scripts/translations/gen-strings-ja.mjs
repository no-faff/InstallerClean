...
const MAP = {
  ...
  'Section.Backup.Folder': `移動先フォルダー`,
  ...
  'Automation.Section.BackupFolder': `移動先フォルダー`,
  'Automation.Section.Patches': `パッチ`,
  'Automation.Section.ProductDetails': `製品詳細`,
  'Automation.BackupFolder': `移動先フォルダー`,
  ...
  'Tooltip.Move': `不要ファイルを移動先フォルダーへ移します。どれも必要とされていないと納得できたら、そのフォルダーを削除してください。`,
  'Tooltip.MoveNeedsDestination': `不要ファイルを移動先フォルダーへ移します。フォルダーはこの後で選びます。どれも必要とされていないと納得できたら、そのフォルダーを削除してください。`,
  'Tooltip.Delete': `不要ファイルを完全に削除します。安全に取り除け、容量はすぐに戻ります。`,
  ...
  'Body.MainExplanation.Action': `選んだ移動先フォルダーへ移し、プログラムがこれまでどおり更新・修復・アンインストールできることを確かめてから、そのフォルダーを削除してください。{InstallerFolder} へ戻せばすべて元どおりになります。あるいは今すぐ完全に削除してください。`,
  ...
  'Body.RegisteredMissingFromDisk': `このインストーラーファイルが見つかりません。今のところ問題はなく、このファイルが属するプログラムを修復・更新・アンインストールしようとする日までは問題になりません。その時点で、Windows がこのファイルを探しても見つからないため、その手順が失敗することがあります。&#10;&#10;直すには、そのプログラムのインストーラーを提供元からダウンロードし、既存のインストールに上書きする形で実行してください(先にアンインストールしないでください。アンインストール自体がこのファイルを必要とする手順です)。入手できるなら、インストール済みのものと同じバージョンを使ってください。別のバージョンは Windows に拒否されることがあります。これでファイルが復元され、設定はそのまま残るはずですが、Microsoft はそれを保証しておらず、Microsoft 自身の最終手段はプログラムの再インストールです。`,
  ...
  'Error.DestinationNotFullyQualified': `移動先フォルダーは、ドライブ文字またはネットワーク共有で始まる、フォルダーへの完全なパスである必要があります(例：D:\\Backup、または \\\\server\\backup)。InstallerClean はこれを使えません：{0}`,
  ...
  'Error.DestinationChangedMidBatch': `InstallerClean は移動先フォルダーを確認できなくなったため、誤った場所に書き込まずに停止しました。{0} を確認してから、再スキャンしてもう一度お試しください。`,
  ...
  'Cli.Help.MoveDefault': `  installerclean-cli /m         保存済みの移動先フォルダーへ移動`,
  ...
  'Tooltip.MoveSameDrive': `不要ファイルを移動先フォルダーへ移します。そのフォルダーは同じドライブにあるため、削除するか別のドライブへ移すまで容量は戻りません。どれも必要とされていないと納得できたら、そうしてください。`,
  'Completion.MoveRestoreHint.Singular': `そのフォルダー内のファイルは[安全に取り除けます]ので、いつでもフォルダーを削除してかまいません。それまでは、万一プログラムが必要とした場合に {InstallerFolder} へ戻せます(可能性は極めて低いです)。`,
  'Completion.MoveRestoreHint.Plural': `そのフォルダー内のファイルは[安全に取り除けます]ので、いつでもフォルダーを削除してかまいません。それまでは、万一プログラムがどれかを必要とした場合に {InstallerFolder} へ戻せます(可能性は極めて低いです)。`,
  'Completion.MoveRestoreHintSameDrive.Singular': `そのフォルダー内のファイルは[安全に取り除けます]ので、実際に容量を取り戻したくなったら、いつでもフォルダーを削除するか別のドライブへ移動してください。それまでは、万一プログラムが必要とした場合に {InstallerFolder} へ戻せます(可能性は極めて低いです)。`,
  'Completion.MoveRestoreHintSameDrive.Plural': `そのフォルダー内のファイルは[安全に取り除けます]ので、実際に容量を取り戻したくなったら、いつでもフォルダーを削除するか別のドライブへ移動してください。それまでは、万一プログラムがどれかを必要とした場合に {InstallerFolder} へ戻せます(可能性は極めて低いです)。`,
  'Confirm.DeletePermanently.Singular': `このファイルは完全に削除されます。[安全に削除できます]が、バックアップが欲しい場合は代わりに「移動」ボタンを使ってください。`,
  'Confirm.DeletePermanently.Plural': `これらのファイルは完全に削除されます。[安全に削除できます]が、バックアップが欲しい場合は代わりに「移動」ボタンを使ってください。`,
  ...
  'Cli.MissingFromDisk.Singular': `{InstallerFolder} にない {0} 個のファイルについて、Windows に登録情報があります：{1}。日常的には支障ありませんが、修復・更新・アンインストールがこれで失敗することがあります。そのプログラムのインストーラーを、できれば同じバージョンで実行し直すと、たいていファイルが復元されます。`,
  'Cli.MissingFromDisk.Plural': `{InstallerFolder} にない {0} 個のファイルについて、Windows に登録情報があります：{1}。日常的には支障ありませんが、修復・更新・アンインストールがこれで失敗することがあります。各プログラムのインストーラーを、できれば同じバージョンで実行し直すと、たいていファイルが復元されます。`,
  ...
  'Cli.DestinationChangedMidBatch': `InstallerClean は移動先フォルダーを確認できなくなったため、誤った場所に書き込まずに停止しました。{0} を確認してから、コマンドをもう一度実行してください。`,
  ...
  'Summary.RecordsNotMatched': `InstallerClean は Windows の登録情報にあるものすべてを突き合わせられなかったため、そのすべては読み取っていません。上の不要ファイルは影響を受けませんが、{InstallerFolder} にないファイルについての記載は全体を示していないことがあります。再スキャンしてもう一度お試しください。`,
  'Cli.RecordsNotMatched': `InstallerClean は Windows の登録情報にあるものすべてを突き合わせられなかったため、そのすべては読み取っていません。見つかったものは影響を受けませんが、{InstallerFolder} にないファイルについての記載は全体を示していないことがあります。もう一度実行すると、さらに見つかることがあります。`,
