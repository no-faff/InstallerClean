...
const MAP = {
  ...
  'Automation.RecycleUnavailable': `不要ファイルの処理方法を選択：安全な場所に移動するか、完全に削除するか、キャンセルするか。`,
  'Automation.RecycleUnavailableMove': `不要ファイルを選択したフォルダーに移動`,
  'Automation.RecycleUnavailableDeletePermanently': `このドライブではごみ箱が利用できないため、不要ファイルを完全に削除`,
  ...
  'Body.RegisteredMissingFromDisk.SeeAlso': `README には、このフォルダーについて、[Microsoft自身の言葉で]説明されており、ファイルの回復方法も記載されています。`,
  'Body.NoPatches': `(なし)`,
  'Reason.Orphaned': `孤立`,
  ...
  'Completion.MoveRestoreHint': `万一何かが動作しなくなったら、{InstallerFolder}にコピーを戻してください ([その可能性は極めて低いです])。`,
  'Completion.DeleteRestoreHint': `それまでは、万一何かが動作しなくなったら復元できます ([その可能性は極めて低いです])。`,
  ...
  'Completion.DeleteSummary.Singular': `{0} 個の {1} をごみ箱に移動しました`,
  'Completion.DeleteSummary.Plural': `{0} 個の {1} をごみ箱に移動しました`,
  ...
  'Completion.PermanentDeleteRestoreHint.Singular': `問題ありません。削除しても安全でした。InstallerCleanはWindowsが不要と報告したファイルのみを削除し、プログラムがまだ必要とするファイルを削除することは決してありません。万が一、削除によってプログラムが修復、更新、またはアンインストールできなくなった場合でも、メーカーから再インストールすることで通常はファイルが復元されます (ただしMicrosoftは保証していません)。`,
  'Completion.PermanentDeleteRestoreHint.Plural': `問題ありません。削除しても安全でした。InstallerClean は Windows が不要と報告したファイルのみを削除し、プログラムがまだ必要とするファイルを削除することは決してありません。万が一、削除によってプログラムが修復、更新、またはアンインストールできなくなった場合でも、メーカーから再インストールすることで通常はファイルが復元されます (ただし Microsoft は保証していません)。`,
  'RecycleUnavailable.Heading': `このドライブではごみ箱が利用できません`,
  'RecycleUnavailable.Body.Singular': `そのため、この {1} ({2})は削除されていません。安全な場所に移動するか、完全に削除することができます。`,
  'RecycleUnavailable.Body.Plural': `そのため、これらの {0} 個の {1} ({2}) は削除されていません。安全な場所に移動するか、完全に削除することができます。`,
  'RecycleUnavailable.Reassurance.Singular': `削除は安全です。InstallerClean は Windows が不要と報告したファイルのみを削除し、プログラムがまだ必要とするファイルを削除することは決してありません。ごみ箱は単なる追加の安全対策です。万が一、削除によってプログラムが修復、更新、またはアンインストールできなくなった場合でも、メーカーから再インストールすることで通常はファイルが復元されます (ただし Microsoft は保証していません)。`,
  'RecycleUnavailable.Reassurance.Plural': `削除は安全です。InstallerClean は Windows が不要と報告したファイルのみを削除し、プログラムがまだ必要とするファイルを削除することは決してありません。ごみ箱は単なる追加の安全対策です。万が一、削除によってプログラムが修復、更新、またはアンインストールできなくなった場合でも、メーカーから再インストールすることで通常はファイルが復元されます (ただし Microsoft は保証していません)。`,
  ...
  'Confirm.DeleteToRecycleBin': `ファイルはごみ箱に移動されます。バックアップコピーが必要な場合は、代わりに「移動」ボタンを使用してください。`,
  ...
  'Error.ShellRecycleFailed': `このファイルをごみ箱に移動できませんでした (エラー{0})。そのコードからは、InstallerClean が理由をお伝えすることはできません。ファイルはそのままにしてあります。代わりに「移動」ボタンをお試しください。「移動」はごみ箱を使いません。`,
  'Error.RecycleAccessDenied': `管理者権限があっても Windows がアクセスを拒否しました (エラー{0})。InstallerClean には、問題がファイルにあるのかごみ箱にあるのかを判別できません。ファイルはそのままにしてあります。問題がごみ箱であれば「移動」ボタンで対処できますが、問題がファイル自体であれば対処できません。`,
  'Error.RecycleInUse': `このファイルは別のプログラムによって開かれているか、ロックされています (エラー{0})。そのため今はどうやっても削除できません。ファイルはそのままにしてあります。しばらくしてからもう一度お試しください。`,
  'Error.DeletedNotRecycled': `Windows はこのファイルをごみ箱に移動せず、完全に削除しました。InstallerClean はごみ箱への移動を要求しましたが、Windows は代わりにこの処理を行いました。ファイルは失われました。`,
  ...
  'Cli.RecycleUnavailable': `エラー：このボリュームではごみ箱が利用できないため、何も削除されませんでした。代わりに/mを使用してファイルを移動するか、ごみ箱を再度有効にして再実行してください。`,
  'Cli.EventLogRecycleUnavailable': `{0}モードは中止されました：Windows ボリュームでごみ箱が利用できません。ファイルは削除されませんでした。`,
  ...
  'Cli.Help.ScanOnly': `  installerclean-cli /s         スキャンのみ - 削除可能なファイルを一覧表示`,
  ...
  'Cli.Help.NoteLine2': `プロンプトをブロックします。他のコンソール exe と同様に、出力をリダイレクト`,
  ...
  'Cli.Help.ExitCodeError': `  1   失敗：何も処理されませんでした (不正な引数、スキャン失敗、すべてのファイルが失敗)`,
  ...
  'Completion.CleanedUp': `{0} クリーンアップ`,
  'Completion.DeleteSpaceHint': `ごみ箱を空にすると、実際に空き容量が増えます。`,
  ...
  'Completion.DeleteCancelledSummary': `キャンセルするまでに、{1} 個中 {0} 個の {2} をごみ箱に移動しました。`,
  ...
  'Cli.Help.MoveScheduledNote': `保存されたデフォルトはユーザーごとの設定です。スケジュール実行や SYSTEM 実行では /m PATH が必要です。`,
  ...
  'Status.CheckingRecycleBin': `ごみ箱を確認中...`,
