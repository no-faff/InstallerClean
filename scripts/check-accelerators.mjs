...
const accelerator = (value, lang) => {
  ...
  return m ? m[1].toLocaleUpperCase(lang) : null;
};
...
if (problems.length) {
  ...
  for (const p of problems) console.error(`  ${p}`);
  console.error('\nFix a clash in the generator (non-repo-files/0-claude/translation/'
    + 'gen-strings-<code>.mjs), then regenerate that satellite. Never hand-edit the resx.');
  process.exit(1);
}
