...
**Optional report.** At the end of a scan you can press "Send report". It does nothing unless you press it, and before anything is sent it shows you the whole thing on screen, so this list is not the thing to trust, the dialog is. It goes to nofaff.netlify.app/api/result-log, a site of mine. What goes: the app version, which of its sixteen languages you were reading it in, and a version number for the report format itself; which Windows you are on and whether it is 64-bit; how long the scan took and how long the run took; how many installer files were registered, orphaned, superseded, obsoleted or missing from disk, how many of the missing ones a program still needs, and how much those add up to in bytes; how many files the scan held back or could not place, and which reason applied to each; whether the run ended as a scan, a move or a delete, and whether it finished; how many files it processed, how many failed, and how many were held back at the last moment with the reason for each; how much space was freed; a count of each kind of error if any happened; and whether a move went to the same drive, another drive, a removable drive or a network share.

It also sends a few facts about the machine itself rather than about the run: how many programs and patches Windows has a record of, whether the machine still makes old-style short file names and how many file names are too long to be one, and counts of any records that were missing, malformed or couldn't be read. Those are there so I can see the shape of real Installer folders and keep making the app more careful about what it holds back. None of them says anything about you.

All of it is numbers and fixed labels. There are no file names, no paths and nothing that identifies you or your machine. Sending it tells my site what the version check tells GitHub, your IP address and the app's name and version, and nothing else. Send one and you are never asked again.
