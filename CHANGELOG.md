...
- Eleven languages now use the words their own Windows uses. A person meeting an unfamiliar tool trusts it more when it speaks in the vocabulary their operating system already taught them, and the app had been using ordinary, correct words that were not always the platform's. Eighty-nine changes, checked against Microsoft's own published terminology, which turns out to be downloadable in bulk with a definition of each sense, so "scan" as in scanning a document could be told apart from "scan" as in searching files. Turkish moved the most: the Turkish Windows uses one root for "update" throughout and the app had been using another, and "installed" had been rendered two different ways inside the same file. German, checked in full, needed nothing at all. Italian, Japanese and Dutch are deliberately untouched, being the work of the people who wrote them rather than machine drafts, so anything found there goes to them instead.
- Three messages in the installer named a button by a word that button does not carry. Turkish and Ukrainian both told you to click a Finish button by a name the wizard does not use, and the Indonesian pair naming the OK and Cancel buttons named them in English while the buttons themselves read Oke and Batal.
