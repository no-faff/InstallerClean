...
- The Registered Files window now flags a registered file that has gone missing from disk: a warning triangle by the product name, "missing" in place of the size, and, when you select the row, a note that Windows still expects the file and which program to reinstall to put it back. These files were always counted among the registered ones (Windows still needs them, they are just absent from disk); now the window shows exactly which they are and how to restore them. The product name comes from the registration, so it survives even though the file's own metadata is gone with the file.
