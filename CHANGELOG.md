...
- Every push now checks the rules that hold between two pieces of text, or between a piece of text and the screen it appears on: that a control's spoken name says what its visible label says, that a sentence quoting a button quotes that button's real label, that the folder placeholder survives translation, and that a window title has not been written twice. Every check the project had looked at one string at a time, which is why the two buttons above could say one thing and speak another in one language and not another, through four releases. Those three were found by this check on its first run.
- The three confirmation dialogs no longer carry a window title string that was replaced before the window could open. Each builds its title from what is on the card, which is what a screen reader reads out when the dialog appears, so the unused strings and their translations have gone from all sixteen languages and from the translation review pages, where native speakers were being asked to check text the app could not show.
...
- Invisible characters that keep `C:\Windows\Installer` on one line are no longer put into text that is only ever spoken and never drawn, where they did nothing and left a speech engine to make sense of them. That holds for every such string now, the two assembled in code included: the line read out when a scan fails, and the title a message dialog is announced by.
...
- The app now tells Windows which language it is drawing in every case, not only when the language has been picked by hand from the language menu. Left on the default Automatic setting it declared US English whatever was on screen, and that declaration is what chooses the substitute font for Japanese, Korean and Chinese characters, which the bundled font does not carry. Those three languages draw some of the same characters differently, so the substitute a machine landed on came down to which East Asian fonts happened to be installed on it rather than to the language being displayed. The other thirteen are unaffected, Windows varying its substitute font by language only across the East Asian character ranges.
