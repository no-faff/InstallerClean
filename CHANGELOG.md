...
- The language picker's tooltip and screen-reader label now read "Change language. The program will restart." rather than "Change the display language. InstallerClean restarts to apply it.": the Windows-specific "display language" wording is dropped (this picker only sets InstallerClean's own language, not the OS one), and the restart note is shortened. English and Italian.
