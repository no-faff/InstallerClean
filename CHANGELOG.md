...
- The two Details windows are retitled in plain descriptive terms rather than bare labels: "Unneeded files that are safe to delete" and "Registered files that must not be deleted", in place of "Unused files" and "Registered files". They are owned, content-sized dialogs, so the longer titles fit, and a screen reader announces what each window is for when it opens.
...
- The README opening now leads with the honest split: only about a third of machines have orphaned installer files to clear, with the median freed around 21 GB and some clearing hundreds of GB, while the other two-thirds find nothing because their Installer folder is already clean and the scan takes seconds either way. The "how much space" example is updated to the measured 1.28 GB and the now-redundant "it could be nothing at all" tail dropped, since the new paragraph covers it.
...
- The operation-status line is left-aligned again. The "Move cancelled." / "Delete cancelled." messages and their partial-cancel variants now sit on the left margin under MOVE LOCATION; a `MaxWidth` on the bound text with the default Stretch alignment had been centring the block in the space beside the buttons, so a short message floated in from the margin. `HorizontalAlignment=Left` pins it to the column.
