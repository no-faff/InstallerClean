...
- The portable download ships uncompressed this release, at roughly double the size (about 135 MB). Its compressed shape has a history of being flagged by Microsoft Defender's machine-learning heuristic as a false positive, it was flagged again on a pre-release test build days after the previous release's copy had been cleared, and a clearance protects only the exact file it was granted for. Rather than publish carrying a flag or wait on another clearance, this release ships the shape with the clean record; the smaller compressed download returns in a point release once its own scan comes back clean.
