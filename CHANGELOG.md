...
- The strings added and reworded in this release, the new error and status messages, the not-yet-scanned and pending-reboot main-window states, the same-drive move warning, the cancelled-operation summaries and the reworded stale-patch and access-denied wording, are now translated in all fourteen languages, so a non-English user sees them in their own language instead of falling back to English.
- The line shown after you cancel a delete now says the files were moved to the Recycle Bin, matching how the rest of the app describes sending files there. It previously said "sent", the one place that wording had drifted.
- Every push now compiles the installer. The setup is built by a single tool, on one machine, at release time, so a mistake in the installer script, or in one of the three community language files bundled with it, could only be found by the person about to ship it, on the day they were shipping it. It is now built and checked on every change.
- A known-vulnerable package now fails the build. The dependency audit that has run on every push since 1.6.0 could not fail anything: the command it used reports a vulnerability and still exits successfully, and each of the four checks was marked "carry on regardless" on top of that, so the audit only ever spoke to whoever went looking in the log. It is now a hard stop, and it covers the packages InstallerClean's own dependencies pull in, which is where a vulnerability actually arrives.
...
- Corrected a misleading comment in both application manifests. It said the version stamped in the manifest is kept matching the one in `Directory.Build.props`, which nothing enforces and which invites whoever cuts a release to keep syncing a value that has no effect: Windows ignores the manifest identity for trust, and InstallerClean reports its version from the assembly, not from there. The comment now records that the value is inert.
...
- The main window no longer tells you to delete files it has just told you do not exist. After a clean-up, and on any scan that finds nothing, it still read "The unneeded files below are safe to delete" and "Delete them to the Recycle Bin, or use Move instead if you'd rather keep a copy" directly above "0 unneeded files to clean up", with Move and Delete greyed out underneath and a live Details button that opened an empty list. That is where the app's most common successful run leaves you. It now says there is nothing to clean up, and the Move and Delete controls go with the files they act on.
...
- Cancelling a Move or a Delete now keeps the record of what it finished before you stopped it. The count of files handled and the list of any that failed used to be discarded along with the cancellation, so the completion screen had nothing to show; they are now handed back, so it can tell you how far the run got.
...
- Immediately before a Move or Delete, InstallerClean now re-confirms with Windows that each file is still one no installed program needs, and leaves out any that has started being needed again since the scan. A cached patch can go from replaced-and-removable back to in-use if the newer patch that replaced it is later uninstalled; had that happened while the results sat on screen, the old file would have been removed even though a program needed it again. It is now re-checked at the moment of acting, and anything that changed is skipped and reported rather than removed.
...
- Delete's messages now say files are "moved to the Recycle Bin" rather than "sent", matching Windows's own dialog. Where a message also offers the separate Move feature (the delete confirmation and the recycle-failure errors), it recommends "the Move button" by name, so "moved" and Move cannot read as one action.
