...
- The reports section is a drawn chart instead of a table of block characters, in all seventeen READMEs, in the same hand as the star history chart below it. One script now owns the whole section and emits both the words and the picture from one banding of the reports, where the picture used to be drawn by a second script that read the first one's table back out of the README to find out where the bands were. The screenshot of what a report contains is no longer hidden behind a disclosure, since the whole section is about what leaves your machine and the evidence should be the part you cannot miss. The sixteen translations keep their own words and show the English chart until theirs are translated: its rows read as numerals and GB, which is most of the way to readable in any language, where a title and two paragraphs of English are readable in none.
- The README now shows what people actually freed, rather than the smallest, the median and the largest. Those three answered almost nothing: what somebody wants to know before running this is how likely they are to get anything and roughly how much. It is a distribution now, seven bands, and the shape is the point: of the 85 people who freed anything, 45 cleared between 10 and 50 GB. The largest single result is named too, 327 GB, in place of "a few cleared hundreds of GB". The reports are described as anonymous as well as optional in that same sentence, which is where a reader first learns the app sends anything at all.
- Two figures in that section were wrong. Each report was rounded to one decimal place before being tested for whether anything was freed, so two people who cleared 4.1 MB and 16.9 MB were counted as having found nothing; the split was 47/53 and is 46/54. Separately the size formatter rounded a figure by hand and then passed the rounded value to a formatter that rounded it again, which stayed invisible for as long as the precision asked for happened to match the first rounding, and turned a median of 19.949676 into 20.0 the moment a decimal place was wanted. Every hand-round is gone and each place that formats a size now says what precision it needs.
- Eleven languages now use the word their own Windows uses. The app had been using ordinary, correct words that were not always the platform's, and somebody meeting an unfamiliar tool trusts it faster when it speaks in vocabulary their operating system already taught them. Eighty-nine changes, each one checked against Microsoft's published terminology, which turns out to be downloadable in full with a definition of every sense, so "scan" as in scanning a document could be told apart from "scan" as in searching files for a match. Turkish moved the most: Turkish Windows builds "update" on one root throughout and the app had consistently used another, and "installed" had been coming out two different ways inside the same file. German needed nothing at all, which was the surprise of the exercise, since the expectation going in was that its word for "scan" would have to go and it turns out to be Microsoft's own. Italian, Japanese and Dutch are untouched on purpose, being written by the people who speak them rather than drafted by machine, so what turned up in those goes to them rather than being changed for them.
...
- The two star history chart files are a third of the size, 70 KB down to 26 KB each, with nothing about the picture changed. Most of each file was the hand-drawn typeface, embedded whole because that is what star-history's own servers have to do: they draw charts for repositories they have never seen and cannot know which characters a legend will need. This repository is known, so only the thirty characters the chart actually sets are carried now. The set is read off the finished drawing rather than guessed, since the axis labels are generated from the data, and a check refuses to write a chart whose typeface cannot set something in it.
- The first row of the reports section reads "Less than 1 GB" where it said "Under 1 GB", which now pairs with the "More than 100 GB" at the other end. The sixteen translations already said their own natural equivalent of "less than", so only the English moved.
...
- Every screenshot in every README now describes the picture that is actually there. The images were all re-taken for 2.3.0 and the descriptions were not, so each one quoted figures from the shot it replaced: the success screen said 69 files where the picture shows 68, and the main window was out on three of its four numbers. That text is only ever read by somebody who cannot see the image, so it is the one place where being out of date is invisible to everyone who could spot it. Eighty-one descriptions across the seventeen, each checked against that language's own screenshots, because the shots were taken separately per language and no two sets carry the same numbers.
- The scan does about a third less work per file, which nobody will notice on an ordinary machine and matters a great deal on the folders people actually report. It was asking Windows to resolve the same folder path once for every file it looked at, rather than once; allocating a fresh half-kilobyte buffer twice per file to hold answers it already had; reading the whole directory twice, once for installers and once for patches, where a folder of several hundred thousand entries makes each pass expensive; and reading every file's size a second time having already been given it. On a folder of 800,000 files that is 8.5 million requests to Windows cut to half that, and the memory the scan churns through drops from 2.4 GB to 827 MB. The list of files offered is identical to the byte, before and after, which is the only part of this that was ever allowed to change.
- A machine whose installed-program records are damaged no longer destroys the crash log while the app reads them. The same fault as the one below, in a different place: reading a product's registration writes a log entry when it fails, which is right for the odd one and wrong when a permissions or registry problem makes every one of them fail. These are real errors carrying stack traces, so each entry is far larger than the scan's, and on a machine with a lot of installed patches the log would turn over several times and take everything else with it. It writes twenty in full now, then one more for each distinct cause, and closes with a count of what it left out.
- A scan that has to turn down every file no longer destroys the crash log while doing it. The check that keeps the app inside `C:\Windows\Installer` writes a log entry each time it refuses a file, which is right for the one or two that normally happens to and wrong when a machine's state makes it refuse the lot: on the very large folders people actually report, that meant one entry per file, tens of thousands of them, and because the log rotates when it fills, everything ever recorded about anything else was pushed out. It now writes twenty in full, then one more for each distinct cause it has not already seen, so a new kind of failure arriving late in the scan is still captured whole, and it finishes with an honest count of how many it left out. Measured over 100,000 refusals: 22 entries where there were 100,000, and nothing older lost.
