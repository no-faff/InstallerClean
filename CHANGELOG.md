...
- After a delete, the completion screen now reads "X cleaned up" rather than "X freed" (a Recycle-Bin delete reclaims nothing until the bin is emptied), adds an "Empty it to actually reclaim the space." line, and ends the restore hint with an "it won't!" link to the README's "Is it safe?" section. The main window widened from 720 to 828 and the completion card from 420 to 520 to fit the longer lines.
