...
- The same refusal now applies to a file whose real location on disk cannot be worked out. The check that every file acted on sits inside `C:\Windows\Installer` expands symlinks and mapped drives first; if that expansion fails, the check was comparing the path as it was written rather than where it leads. Being unable to prove a file is inside the cache now stops it being touched, exactly as being outside does.
- The window raised when you launch InstallerClean while it is already running is now signalled within your own Windows session. On a machine with fast user switching, a terminal server or a reconnected remote desktop, that signal could reach a copy running in someone else's session: the window rose on a desktop you cannot see, and the "already running" message was suppressed because the signal had been delivered, so a launch looked like nothing at all. A launch from another session now gets the message. The single-instance lock is unchanged and still covers the whole machine, because the folder it protects is shared by every session.
...
- The build check that compiles the installer on every push now bundles the same command-line tool the released installer carries. It was compiling around the lighter build that needs the .NET Desktop Runtime installed, so the one artefact the check exists to prove was not the one that ships.
- Improved code comments: the machine-wide cost of holding the Windows Installer lock is now recorded beside both places that take it, along with why closing the window during an operation waits on the flag it does, why the re-check before a clean-up re-reads everything rather than just the files in hand, and the reasoning behind each of the safety changes above. Also tidied five places in the command-line tool that passed the same sentence twice to the wording helper, which is the shape it was given a plainer form to retire.
