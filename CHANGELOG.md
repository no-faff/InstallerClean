...
- The send-confirmation window shows the whole report with no scrollbar. Every successful run's report is the same 25 lines, but the preview box was capped a line or two shy of them, so the report always ended under a scrollbar. The box now sizes to the report and the window is clamped to the screen's working height, so a scrollbar appears only when the report genuinely cannot fit: an error-heavy report or a very large text scale.
- The send-confirmation caption leads with the privacy fact instead of the URL. It read "Goes to nofaff.netlify.app/api/result-log. Counts and labels only, nothing identifying you or your machine. Helps me know whether the app is working.", putting where it goes, the least important part, first. It now reads "Nothing identifies you or your machine; it just lets me know the app is working and how much space people are freeing. It goes to nofaff.netlify.app/api/result-log."
...
- The READMEs are reconciled with the 1.9.0 delete and CLI behaviour. The delete copy now matches the conditional Recycle Bin behaviour, that Delete sends files to the bin and, when the bin is unavailable for the drive, refuses to remove them for good on its own, offering Move or a permanent delete you confirm; the old unconditional "goes to the Recycle Bin, restore from there" promise is softened across the intro, the "Is it safe?" passage and the undo-a-delete FAQ. That nuance is now single-sourced in the "Is it safe?" passage so the four locales cannot drift, and the CLI exit codes are corrected (a `/s` scan is 0 on success, 1 on failure and 130 on Ctrl+C, not "always 0").
- The README stops selling Move as the safety. The delete copy located the safety in Move ("Move lets you keep them somewhere safe", "Move is the extra-safe option", "For a safety net you control, use Move"), which undersold the actual basis: the app only ever lists files Windows itself reports as finished with, so Delete is the normal, intended action. The three passages now say exactly that, with Move presented as the option for anyone who'd rather see for themselves first, parking the files in a folder of their choice.
...
- The recycle probe cleans up after itself. Before deleting, the app checks the Recycle Bin really works on the volume by recycling a tiny test file (`ic-recycle-probe-*.tmp`), and on success that ~1 KB entry used to stay in the bin, a puzzling stranger among the user's own deletions. The shell reports the exact bin entry a delete creates (the progress sink's `psiNewlyCreated`); the probe now captures that item and permanently deletes it after a successful test. Addressed by identity rather than by guessing at `$Recycle.Bin` paths, the cleanup can only ever remove the probe's own entry; real deletions are untouched and stay restorable, and if the tidy-up fails the entry stays behind, which is no worse than the old behaviour.
...
- The Delete confirmation chain drops its dead size parameters. Removing the large-delete warning left `totalBytes` and `maxSingleFileBytes` flowing through the window constructor, `IConfirmationService`, `ConfirmationService` and the `CleanupViewModel` call only to be ignored; the signatures now carry exactly what the dialog reads, and the test call-sites follow.
- The P/Invoke declarations the shell-view launcher rework orphaned are removed: `GetShellWindow`, `OpenProcess`, `CloseHandle` and `PROCESS_QUERY_INFORMATION` served the `CreateProcessWithTokenW` token-duplication launcher, and nothing reads them since the shell-view dispatch chain replaced it. A smaller declared import surface is also one less thing for an AV heuristic to weigh.
