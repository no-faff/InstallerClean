...
- The README opening now stands alone: what the app does and that it's a run-once tool, the reports' live split (the percentages and the 23 GB median are script-written, so they track the table), a safety bullet naming the Windows Installer API as the source of truth alongside the open-source and nothing-leaves-your-machine stance, and a download line. Replaces the per-machine anecdotes and the earlier What/space/safe/get bullets.
