...
- The "Preparing destination folder..." overlay no longer flashes before the Move confirmation when the destination is local; it waits 200 ms and appears only if the destination is slow to probe (a network share stalling on its timeout), the way the scan defers its own overlay. The cancellable probe is unchanged.
