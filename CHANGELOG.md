...
- **Send result** on the completion screen. After every all-clear scan, Move or Delete that produces a fresh result, the app writes a small JSON file (`%LOCALAPPDATA%\NoFaff\InstallerClean\last-run.json`) summarising the operation in counts and categorical labels only: schema version, app version, OS string, scan duration, registered / orphaned / superseded / missing counts, pending-reboot label, operation kind, outcome, files processed, files failed, bytes freed, error buckets and (for moves) destination kind. No file paths, no user names, no machine identifiers, no time-of-day. Clicking the button opens a confirmation window showing the literal JSON about to be POSTed; the user reviews and presses Send to actually transmit. The Send prompt appears at most once per session and at most once ever per user (a `HasSentResultLog` flag on `AppSettings` locks it after the first successful send, so the cohort of reports we receive is each user's first ever, never a low-impact rerun). Tooltip switches to the "please send even though nothing was found" variant on all-clear runs so the zero-result cohort isn't quietly filtered out of the aggregate.
- "SAY THANKS" section header above the Star + Donate buttons in About, with Star renamed to "Leave a star on GitHub" to match.

### Changed

- The completion-screen "Share what you cleared" button (browser-mediated cleared-bytes share) was replaced before release by Send result, which does the network call directly rather than handing the value off through a browser confirmation page. The JSON content is what gets sent and the user reviews it in a modal before pressing Send.
- All-clear overlay heading reads "All clean" instead of "All clear".
- Completion heading after a successful Move or Delete reads "{N} freed" instead of "{N} cleared".
- Send-failure status copy switched from "Try again in a few minutes" to "Didn't work. Never mind, click Done." (and "Network error. Never mind, click Done." for unreachable / timed-out cases). The button is hidden for the rest of the session after any click so the user isn't asked to retry what was a favour.
