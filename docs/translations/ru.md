...
| BACKUP FOLDER | ПАПКА НАЗНАЧЕНИЯ |
...
| Move them to a backup folder you choose, then delete that folder when you're satisfied your programs still update, repair and uninstall as normal. Putting them back into {InstallerFolder} restores everything. Or delete them permanently now. | Переместите их в выбранную вами папку назначения, а затем удалите эту папку, когда убедитесь, что ваши программы по-прежнему обновляются, восстанавливаются и удаляются как обычно. Возврат их в {InstallerFolder} восстанавливает всё. Или удалите их безвозвратно прямо сейчас. |
...
| Windows has a file rename queued for the next restart that affects {InstallerFolder}. Restart Windows before cleaning. | Windows поставила в очередь на следующую перезагрузку переименование файла, которое затрагивает {InstallerFolder}. Перезагрузите Windows, прежде чем очищать. |
...
| --- | --- |
| All clean | Всё чисто |
| Nothing removed | Ничего не убрано |
...
| InstallerClean couldn't be certain which cached files belong to the programs installed here, so it has held back the one file ({1}) it might otherwise have offered. | InstallerClean не смог с уверенностью определить, каким из установленных здесь программ принадлежат файлы в кэше, поэтому удержал единственный файл ({1}), который иначе предложил бы. |
| InstallerClean couldn't be certain which cached files belong to the programs installed here, so it has held back all {0} files ({1}) it might otherwise have offered. | InstallerClean не смог с уверенностью определить, каким из установленных здесь программ принадлежат файлы в кэше, поэтому удержал все {0} файлов ({1}), которые иначе предложил бы. |
| The file in that folder is [safe to remove], so delete the folder whenever you want. Until then, you can put it back into {InstallerFolder} if a program ever turns out to need it (extremely unlikely). | Файл в этой папке [можно безопасно убрать], так что удаляйте папку когда захотите. До тех пор вы можете вернуть его в {InstallerFolder}, если какой-нибудь программе он всё же понадобится (крайне маловероятно). |
| The files in that folder are [safe to remove], so delete it whenever you want. Until then, you can put them back into {InstallerFolder} if a program ever turns out to need one (extremely unlikely). | Файлы в этой папке [можно безопасно убрать], так что удаляйте её когда захотите. До тех пор вы можете вернуть их в {InstallerFolder}, если какой-нибудь программе понадобится один из них (крайне маловероятно). |
| The file in that folder is [safe to remove], so delete the folder or move it to another drive whenever you want to actually reclaim the space. Until then, you can put it back into {InstallerFolder} if a program ever turns out to need it (extremely unlikely). | Файл в этой папке [можно безопасно убрать], так что удалите папку или переместите её на другой диск, когда действительно захотите вернуть место. До тех пор вы можете вернуть его в {InstallerFolder}, если какой-нибудь программе он всё же понадобится (крайне маловероятно). |
| The files in that folder are [safe to remove], so delete it or move it to another drive whenever you want to actually reclaim the space. Until then, you can put them back into {InstallerFolder} if a program ever turns out to need one (extremely unlikely). | Файлы в этой папке [можно безопасно убрать], так что удалите её или переместите на другой диск, когда действительно захотите вернуть место. До тех пор вы можете вернуть их в {InstallerFolder}, если какой-нибудь программе понадобится один из них (крайне маловероятно). |
...
| {0} {1} moved to: {2} | Перемещено {0} {1} в: {2} |
| {0} {1} kept in place, because the records now claim what the scan flagged. | Оставлено на месте {0} {1}, потому что записи теперь заявляют то, что отметило сканирование. |
| {0} {1} kept in place, because the Windows Installer records had changed by the final check. | Оставлено на месте {0} {1}, потому что к итоговой проверке записи Windows Installer изменились. |
| {0} {1} kept in place, because the Windows Installer records could not be fully read in the final check. | Оставлено на месте {0} {1}, потому что при итоговой проверке записи Windows Installer не удалось прочитать полностью. |
| {0} {1} kept in place, because by the final check InstallerClean could not be certain which cached files belong to the programs installed here. | Оставлено на месте {0} {1}, потому что к итоговой проверке InstallerClean не смог с уверенностью определить, каким из установленных здесь программ принадлежат файлы в кэше. |
...
| Windows has a record for {0} file that isn't in {InstallerFolder}: {1}. It causes no trouble day to day, but a repair, update or uninstall can fail on it. Open Details for what to do. | У Windows есть запись о {0} файле, которого нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но восстановление, обновление или удаление программы из-за него может не удаться. Откройте «Подробности», чтобы узнать, что делать. |
| Windows has records for {0} files that aren't in {InstallerFolder}: {1}. They cause no trouble day to day, but a repair, update or uninstall can fail on them. Open Details for what to do. | У Windows есть записи о {0} файлах, которых нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но восстановление, обновление или удаление программы из-за них может не удаться. Откройте «Подробности», чтобы узнать, что делать. |
...
| InstallerClean couldn't match up everything in the Windows records, so it didn't read all of them. The unneeded files above are unaffected, but what it says about files missing from {InstallerFolder} may be short of the full picture. Re-scan to try again. | InstallerClean не смог сопоставить всё, что есть в записях Windows, поэтому прочитал их не полностью. Ненужных файлов выше это не касается, но сказанное о файлах, отсутствующих в {InstallerFolder}, может быть неполным. Повторите сканирование, чтобы попробовать снова. |
| {0} of {1} {2} | {0}/{1} {2} |
...
| This file will be deleted permanently. It's [safe to delete], but if you'd like a backup, use the Move button instead. | Этот файл будет удалён безвозвратно. Его [можно безопасно удалить], но если хотите резервную копию, воспользуйтесь кнопкой «Переместить». |
| Files will be deleted permanently. They're [safe to delete], but if you'd like a backup, use the Move button instead. | Файлы будут удалены безвозвратно. Их [можно безопасно удалить], но если хотите резервную копию, воспользуйтесь кнопкой «Переместить». |
...
| Move the unneeded files to the backup folder. Delete that folder whenever you're satisfied nothing needs them. | Перемещает ненужные файлы в папку назначения. Удалите эту папку, когда убедитесь, что они никому не нужны. |
| Move the unneeded files to a backup folder. You'll choose it next. Delete that folder whenever you're satisfied nothing needs them. | Перемещает ненужные файлы в папку назначения. Вы выберете её следующим шагом. Удалите эту папку, когда убедитесь, что они никому не нужны. |
| Move the unneeded files to the backup folder. It's on the same drive, so you won't reclaim the space until you delete that folder or move it to another drive. You can do that whenever you're satisfied nothing needs them. | Перемещает ненужные файлы в папку назначения. Она на том же диске, поэтому место вернётся только после того, как вы удалите эту папку или переместите её на другой диск. Это можно сделать, когда убедитесь, что они никому не нужны. |
| Delete the unneeded files permanently. They're safe to remove, and you'll reclaim the space straight away. | Безвозвратно удаляет ненужные файлы. Их можно безопасно убрать, и место вернётся сразу. |
...
| Windows has a record for {0} file that is not in {InstallerFolder}: {1}. It causes no trouble day to day, but a repair, update or uninstall can fail on it. Running that program's installer again, preferably the same version, usually restores the file. | У Windows есть запись о {0} файле, которого нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но восстановление, обновление или удаление программы из-за него может не удаться. Повторный запуск установщика той программы, желательно той же версии, обычно возвращает файл. |
| Windows has records for {0} files that are not in {InstallerFolder}: {1}. They cause no trouble day to day, but a repair, update or uninstall can fail on them. Running each program's installer again, preferably the same version, usually restores the files. | У Windows есть записи о {0} файлах, которых нет в {InstallerFolder}: {1}. В повседневной работе это не мешает, но восстановление, обновление или удаление программы из-за них может не удаться. Повторный запуск установщика каждой программы, желательно той же версии, обычно возвращает файлы. |
| InstallerClean couldn't match up everything in the Windows records, so it didn't read all of them. What it found is unaffected, but what it says about files missing from {InstallerFolder} may be short of the full picture. Running it again may pick up more. | InstallerClean не смог сопоставить всё, что есть в записях Windows, поэтому прочитал их не полностью. Найденного это не касается, но сказанное о файлах, отсутствующих в {InstallerFolder}, может быть неполным. Повторный запуск, возможно, найдёт больше. |
| Deleting {0} unneeded {1}... | Удаление {0} ненужных {1}... |
...
| Moving {0} unneeded {1} to {2}... | Перемещение {0} ненужных {1} в {2}... |
| Moved {0} unneeded {1}. | Перемещено {0} ненужных {1}. |
| InstallerClean could no longer confirm the backup folder, so it stopped rather than write into the wrong place. Check {0}, then run the command again. | InstallerClean больше не смог подтвердить папку назначения и остановился, чтобы не записать не туда. Проверьте {0}, затем запустите команду снова. |
...
| Removes cached .msi and .msp files that no installed program still needs. | Убирает файлы .msi и .msp из кэша, не нужные ни одной программе. |
...
| That folder is saved per-user; scheduled or SYSTEM runs need /m PATH. | Папка хранится для каждого пользователя; заданиям нужен /m ПУТЬ. |
