...
| Windows Installer refused to list the installed products, and InstallerClean is already running as administrator, so running it again won't help. The permissions on the Windows Installer records may have been changed, or security software may be blocking them. Running 'sfc /scannow' from an elevated prompt is worth a try. | Windows Installer odmówił wypisania zainstalowanych produktów, a InstallerClean działa już z uprawnieniami administratora, więc ponowne uruchomienie nic nie da. Uprawnienia do rekordów samego Windows Installera mogły zostać zmienione albo blokuje je oprogramowanie zabezpieczające. Warto spróbować uruchomić „sfc /scannow” z wiersza poleceń z podwyższonymi uprawnieniami. |
