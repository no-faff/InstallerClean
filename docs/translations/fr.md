...
| Windows Installer refused to list the installed products, and InstallerClean is already running as administrator, so running it again won't help. The permissions on the Windows Installer records may have been changed, or security software may be blocking them. Running 'sfc /scannow' from an elevated prompt is worth a try. | Windows Installer a refusé de lister les produits installés, et InstallerClean s'exécute déjà en tant qu'administrateur, donc le relancer n'y changera rien. Les autorisations sur les enregistrements de Windows Installer ont peut-être été modifiées, ou un logiciel de sécurité les bloque. Exécuter « sfc /scannow » depuis une invite élevée vaut la peine d'être tenté. |
