...
| Windows Installer refused to list the installed products, and InstallerClean is already running as administrator, so running it again won't help. The permissions on the Windows Installer records may have been changed, or security software may be blocking them. Running 'sfc /scannow' from an elevated prompt is worth a try. | O Windows Installer recusou-se a listar os produtos instalados, e o InstallerClean já está em execução como administrador, então executá-lo de novo não vai ajudar. As permissões dos registros do Windows Installer podem ter sido alteradas, ou algum software de segurança pode estar bloqueando o acesso. Vale a pena executar 'sfc /scannow' em um prompt elevado. |
